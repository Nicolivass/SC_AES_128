import logging
from argparse import ArgumentParser
from enum import Enum


#tabela de substituição da subBytes
s_box = (
    
)

#tabela inversa de substituição para decifrar
inversa_s_box = (
    
)

#constantes da expansão de chaves 
round_constants = (
    
)

matriz_mix_columns = (
    (0x02, 0x03, 0x01, 0x01),
    (0x01, 0x02, 0x03, 0x01),
    (0x01, 0x01, 0x02, 0x03),
    (0x03, 0x01, 0x01, 0x02)
)

matriz_inversa_mix_columns = (
    (0x0E, 0x0B, 0x0D, 0x09),
    (0x09, 0x0E, 0x0B, 0x0D),
    (0x0D, 0x09, 0x0E, 0x0B),
    (0x0B, 0x0D, 0x09, 0x0E)
)

#exponenciação do galois field
E = (
    
)

#logaritmo em galois field
L = (
    -1, 
)


def shift_rows(state):
    return [
        state[0], state[5], state[10], state[15],
        state[4], state[9], state[14], state[3],
        state[8], state[13], state[2], state[7],
        state[12], state[1], state[6], state[11]
    ]


def inv_shift_rows(state):
    return [
        state[0], state[13], state[10], state[7],
        state[4], state[1], state[14], state[11],
        state[8], state[5], state[2], state[15],
        state[12], state[9], state[6], state[3]
    ]


def add_round_key(state, round_key):
    return [state[i] ^ round_key[i] for i in range(16)]


#multiplicação de 2 elementos no campo finito (galois field)
def galois_mult(a, b):
    if a == 0 or b == 0:
        return 0
    if a == 1 or b == 1:
        return a * b

    return E[(L[a] + L[b]) % 0xff]


def mix_columns(state, matriz):
    for i in range(4):
        column = state[i * 4: i * 4 + 4]
        state[i * 4] = galois_mult(matriz[0][0], column[0]) ^ galois_mult(matriz[0][1], column[1]) ^ galois_mult(matriz[0][2], column[2]) ^ galois_mult(matriz[0][3], column[3])
        state[i * 4 + 1] = galois_mult(matriz[1][0], column[0]) ^ galois_mult(matriz[1][1], column[1]) ^ galois_mult(matriz[1][2], column[2]) ^ galois_mult(matriz[1][3], column[3])
        state[i * 4 + 2] = galois_mult(matriz[2][0], column[0]) ^ galois_mult(matriz[2][1], column[1]) ^ galois_mult(matriz[2][2], column[2]) ^ galois_mult(matriz[2][3], column[3])
        state[i * 4 + 3] = galois_mult(matriz[3][0], column[0]) ^ galois_mult(matriz[3][1], column[1]) ^ galois_mult(matriz[3][2], column[2]) ^ galois_mult(matriz[3][3], column[3])
    return state

#rotação à esquerda
def rotaciona_word(word):
    return word[1:] + word[:1]


def sub_bytes(state):
    return [s_box[b] for b in state]


def inversa_sub_bytes(state):
    return [inversa_s_box[b] for b in state]

#geração de chave expandida
def expand_key(key, rounds):
    expanded_key = []
    expanded_key += key

    for i in range(4, 4 * rounds):
        temp = expanded_key[-4:]
        if i % 4 == 0:
            temp = rotaciona_word(temp)
            temp = sub_bytes(temp)
            temp[0] ^= round_constants[(i // 4) - 1]

        for j in range(4):
            temp[j] ^= expanded_key[-16 + j]

        expanded_key += temp
    return expanded_key

#criptografia de um único bloco 
def cifragem_bloco(state: bytes, expanded_key: bytes, rounds: int):
    geração_round_key = (expanded_key[i * 16: i * 16 + 16] for i in range(rounds))
    state = add_round_key(state, next(geração_round_key))
    if rounds <= 1:
        return state
    
    #se tiver mais de 1 rodada
    rounds_meio = rounds - 2
    for _ in range(rounds_meio):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state, matriz_mix_columns)
        state = add_round_key(state, next(geração_round_key))
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, next(geração_round_key))
    return state


def bytes_to_hex_string(bytes: bytes):
    return ''.join(list(map(lambda x: f'{x:0>2x}', bytes)))

#preenche mensagem até comprimento ser múltiplo do tamanho
def preenche_messagem(message: bytes):
    block_size = 16
    padding = block_size - len(message) % block_size
    if padding == 0:
        padding = block_size
    logging.info(f'Preenchendo mensagem com {padding} bytes')
    padded_message = message + bytes([padding] * padding)
    return padded_message

#tira o preenchimento adicionado
def restaura_mensagem(message):
    padding = message[-1]
    return message[:-padding]

#junta uma lista de blocos
def combina_blocos(states: list[list[bytes]]):
    return bytes([byte for state in states for byte in state])

#divide mensagem em blocos menores
def divide_mensagem(message, block_size=16):
    return [message[i:i+block_size] for i in range(0, len(message), block_size)]

#gera o fluxo de chaves para o CTR
def gera_fluxo_chaves(expanded_key: bytes, rounds: int, iv: bytes, n: int):
    keystreams = []
    logging.info(f'IV: {bytes_to_hex_string(iv)}')
    counter = int.from_bytes(iv, byteorder='big')
    for _ in range(n):
        counter_block = counter.to_bytes(16, byteorder='big')
        keystream = cifragem_bloco(counter_block, expanded_key, rounds)
        keystreams.append(keystream)
        counter += 1
    return keystreams


def cifragem_ctr(states, expanded_key, rounds, iv):
    if iv is None:
        raise Exception('Vetor de Inicialização é necessário para o modo CTR')

    keystreams = gera_fluxo_chaves(expanded_key, rounds, iv, len(states))

    blocks = []
    for plain_text_block, keystream in zip(states, keystreams):
        blocks.append(
            [a ^ b for a, b in zip(plain_text_block, keystream)])
    return blocks


def cifra(message: bytes, key: bytes, iv: bytes = None, rounds=11):
    expanded_key = expand_key(key, rounds)
    padded_message = preenche_messagem(message)
    states = divide_mensagem(padded_message)

    logging.info('Mensagem: ' + bytes_to_hex_string(message))
    logging.info('Rodadas: ' + str(rounds))
    logging.info('Chave: ' + bytes_to_hex_string(key))
    logging.info('Numero de Blocos: ' + str(len(states)))

    encrypted_blocks = cifragem_ctr(states, expanded_key, rounds, iv)

    encrypted_message = combina_blocos(encrypted_blocks)

    logging.info('Mensagem Cifrada: ' +
                 bytes_to_hex_string(encrypted_message))

    return encrypted_message


def decifra_bloco(state: bytes, expanded_key: bytes, rounds=11):
    geração_round_key = (expanded_key[i * 16: i * 16 + 16]
                     for i in range(rounds - 1, -1, -1))
    state = add_round_key(state, next(geração_round_key))
    if rounds <= 1:
        return state
    rounds_meio = rounds - 2
    for _ in range(rounds_meio):
        state = inv_shift_rows(state)
        state = inversa_sub_bytes(state)
        state = add_round_key(state, next(geração_round_key))
        state = mix_columns(state, matriz_inversa_mix_columns)
    state = inv_shift_rows(state)
    state = inversa_sub_bytes(state)
    state = add_round_key(state, next(geração_round_key))
    return state


def decifra(message: bytes, key: bytes, iv: bytes = None, rounds=11):
    expanded_key = expand_key(key, rounds)
    states = divide_mensagem(message)

    logging.info('Mensagem: ' + bytes_to_hex_string(message))
    logging.info('Rodadas: ' + str(rounds))
    logging.info('Chave: ' + bytes_to_hex_string(key))
    logging.info('Numero de Blocos: ' + str(len(states)))

    decrypted_blocks = cifragem_ctr(states, expanded_key, rounds, iv)

    decrypted_message = combina_blocos(decrypted_blocks)
    decrypted_message = restaura_mensagem(decrypted_message)

    logging.info('Mensagem Decifrada: ' +
                 bytes_to_hex_string(decrypted_message))

    return decrypted_message

#arruma o comprimento
def ajusta_string(string: str, length: int):
    if len(string) < length:
        logging.info(
            'string hexadecimal muito curta, preenchendo com zero até o comprimento desejado')
        return string.ljust(length, '0')

    if len(string) > length:
        logging.info(
            'string hexadecimal muito longa, ignorando o excesso')
        return string[:length]

    return string


if __name__ == '__main__':
    parser = ArgumentParser()
    parser.add_argument('-in', '--input_file', type=str, required=True)
    parser.add_argument('-out', '--output_file', type=str, required=True)
    parser.add_argument(
        '-e', '--encrypt', action='store_true', dest='encrypt_flag')
    parser.add_argument(
        '-d', '--decrypt', action='store_true', dest='decrypt_flag')
    parser.add_argument('-k', '--key', type=str, required=True)
    parser.add_argument('-iv', type=str)
    parser.add_argument('-r', '--rounds', type=int, default=11)
    parser.add_argument(
        '-v', '--verbose', action='store_true', dest='verbose_flag')
    args = parser.parse_args()

    if args.verbose_flag:
        logging.basicConfig(level=logging.INFO)

    input_file = args.input_file
    output_file = args.output_file
    encrypt_flag = args.encrypt_flag
    decrypt_flag = args.decrypt_flag
    key = bytes.fromhex(ajusta_string(args.key, 32))
    iv = bytes.fromhex(ajusta_string(args.iv, 32)) if args.iv else None
    rounds = args.rounds

    if encrypt_flag or decrypt_flag:
        with open(input_file, 'rb') as f:
            message = f.read()

            if encrypt_flag:
                output_data = cifra(message, key, iv, rounds)
            else:
                output_data = decifra(message, key, iv, rounds)
            with open(output_file, 'wb') as f:
                f.write(output_data)
